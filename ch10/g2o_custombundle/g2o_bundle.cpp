...
void SetSparseSchurSolver(BALProblem* bal_problem, const BundleParams& params, g2o::SparseOptimizer* optimizer)
{
    //std::cout<<"Set Sparse Schur Solver..."<<std::endl;
    ...
    linearSolver = std::make_unique<g2o::LinearSolverCholmod<BalBlockSolver::PoseMatrixType>>();
    dynamic_cast<g2o::LinearSolverCholmod<BalBlockSolver::PoseMatrixType>* >(linearSolver.get())->setBlockOrdering(true);  // AMD ordering , only needed for sparse cholesky solver
    

    std::unique_ptr<BalBlockSolver> solver_ptr = std::make_unique<BalBlockSolver>(std::move(linearSolver));
...
void SetDenseSchurSolver(BALProblem* bal_problem, const BundleParams& params, g2o::SparseOptimizer* optimizer)
{
    //std::cout<<"Set Dense Schur Solver..."<<std::endl;
    std::unique_ptr<g2o::LinearSolver<BalBlockSolver::PoseMatrixType>> linearSolver;
    linearSolver = std::make_unique<g2o::LinearSolverDense<BalBlockSolver::PoseMatrixType>>();
...
void SetIterativeSchurSolver(BALProblem* bal_problem, const BundleParams& params, g2o::SparseOptimizer* optimizer)
{
    //std::cout<<"Set Iterative Schur Solver..."<<std::endl;
    std::unique_ptr<g2o::LinearSolver<BalBlockSolver::PoseMatrixType>> linearSolver;
    if(params.linear_solver == "conjugate_gradient"){
        linearSolver = std::make_unique<g2o::LinearSolverPCG<BalBlockSolver::PoseMatrixType>>();
        // 修复：g2o::LinearSolverPCG 没有 setPCGType 方法，已移除
    }
    else{
        linearSolver = std::make_unique<g2o::LinearSolverEigen<BalBlockSolver::PoseMatrixType>>();
    }
...
void SetLinearSolver(std::unique_ptr<BalBlockSolver>& solver_ptr, const BundleParams& params)
{
    ...
    if(params.linear_solver == "dense_schur"){
        ...
        std::unique_ptr<g2o::LinearSolver<BalBlockSolver::PoseMatrixType>> linearSolver;
        linearSolver = std::make_unique<g2o::LinearSolverDense<BalBlockSolver::PoseMatrixType>>();
        solver_ptr = std::make_unique<BalBlockSolver>(std::move(linearSolver));
    }
    ...
    /*
    else if(params.linear_solver == "iterative_schur"){
        solver_ptr.reset();
        std::unique_ptr<g2o::LinearSolver<BalBlockSolver::PoseMatrixType>> linearSolver;
        // 由于BundleParams中没有preconditioner成员，我们注释掉这部分代码
        // if(params.preconditioner == "jacobi"){
        //     linearSolver = std::make_unique<g2o::LinearSolverPCG<BalBlockSolver::PoseMatrixType>>();
        // }
        // else if(params.preconditioner == "schur_jacobi"){
        //     linearSolver = std::make_unique<g2o::LinearSolverEigen<BalBlockSolver::PoseMatrixType>>();
        // }
        // else{
        //     std::cout<<"Please check your preconditioner parameter again.."<<std::endl;
        //     exit(EXIT_FAILURE);
        // }
        // 使用默认的LinearSolverEigen
        linearSolver = std::make_unique<g2o::LinearSolverEigen<BalBlockSolver::PoseMatrixType>>();
        solver_ptr = std::make_unique<BalBlockSolver>(std::move(linearSolver));
    }
    */
    else if(params.linear_solver == "conjugate_gradient"){
        solver_ptr.reset();
        std::unique_ptr<g2o::LinearSolver<BalBlockSolver::PoseMatrixType>> linearSolver;
        linearSolver = std::make_unique<g2o::LinearSolverPCG<BalBlockSolver::PoseMatrixType>>();
        solver_ptr = std::make_unique<BalBlockSolver>(std::move(linearSolver));
        ...
        std::cout<<"Please check your linear_solver parameter again.."<<std::endl;
        exit(EXIT_FAILURE);
    }
...
{
    //std::cout<<"Set Solver Options From Flags..."<<std::endl;
    std::unique_ptr<g2o::LinearSolver<BalBlockSolver::PoseMatrixType>> linearSolver;
    if(params.linear_solver == "dense_schur"){
        linearSolver = std::make_unique<g2o::LinearSolverDense<BalBlockSolver::PoseMatrixType>>();
    }
    ...
    */
    ...
    auto solver_ptr = std::make_unique<BalBlockSolver>(std::move(linearSolver));
    g2o::OptimizationAlgorithmWithHessian* algorithm;
    if (params.trust_region_strategy == "levenberg_marquardt") {
        algorithm = new g2o::OptimizationAlgorithmLevenberg(std::move(solver_ptr));
    } else if (params.trust_region_strategy == "dogleg") {
        algorithm = new g2o::OptimizationAlgorithmDogleg(std::move(solver_ptr));
    } else {
        std::cout << "Please check your trust_region_strategy parameter again.." << std::endl;
        exit(EXIT_FAILURE);
    }
